...
export class Grid {
  ...
  generateQualityMap(getTileConfig: (tileId: string) => TileConfig | undefined) {
    ...
    // === ШАР 6: НОВА КІМНАТА (для тестування виправленої hit area) ===
    this.generateRoomWithCorners(
      { x: 110, y: 110 }, // Позиція нової кімнати
      { width: 12, height: 12 }, // Квадратна форма 12×12
      { x: 115, y: 110 }, // Прохід (двері) - північ, по центру
      getTileConfig
    );

    console.log('✅ Карта згенерована (floor + water + forest + DIRT + barrel + нова кімната 12×12)!');
  }
