...
export class Grid {
  ...
  generateQualityMap(getTileConfig: (tileId: string) => TileConfig | undefined) {
    ...
    // === ШАР 6: ТЕСТОВА КІМНАТА (одна проста кімната для тестування позиціонування стін) ===
    this.generateRoomWithCorners(
      { x: 100, y: 100 }, // Позиція кімнати
      { width: 10, height: 10 }, // Квадратна форма 10×10
      { x: 104, y: 100 }, // Прохід (двері) - північ, по центру
      getTileConfig
    );

    console.log('✅ Карта згенерована (floor + water + forest + DIRT + barrel + 1 тестова кімната)!');
  }
