...
export class Grid {
  ...
  generateQualityMap(getTileConfig: (tileId: string) => TileConfig | undefined) {
    ...
    // === ШАР 6: НОВА КІМНАТА 1 (для тестування ground tiles та напівпрозорих стін) ===
    this.generateRoomWithCorners(
      { x: 100, y: 100 }, // Позиція першої кімнати
      { width: 14, height: 10 }, // Прямокутна форма 14×10
      { x: 106, y: 100 }, // Прохід (двері) - північ, по центру
      getTileConfig
    );

    // === ШАР 7: НОВА КІМНАТА 2 (для тестування ground tiles та напівпрозорих стін) ===
    this.generateRoomWithCorners(
      { x: 120, y: 120 }, // Позиція другої кімнати
      { width: 10, height: 14 }, // Прямокутна форма 10×14 (вертикальна)
      { x: 124, y: 120 }, // Прохід (двері) - північ, по центру
      getTileConfig
    );

    console.log('✅ Карта згенерована (floor + water + forest + DIRT + barrel + 2 нові кімнати для тестування ground tiles)!');
  }
